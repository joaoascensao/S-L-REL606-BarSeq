...
gg = list(set(list(pathlen_df['source'])))
geneAs=[]
gn_genes = list(set(needed_genes).intersection(set(gg)))
for i in range(boot):
	a = random.choice(gn_genes)
	geneAs.append(a)
							...
							if geneA in comm2:
								...
								if geneC in comm2:
									...
									for i2b,comm2 in enumerate(all_communities[strain2]):
										if geneC in comm2:
											break
								#row = pathlen_df[((pathlen_df['source']==geneA) & (pathlen_df['target']==geneC)) | 
								#	((pathlen_df['source']==geneC) & (pathlen_df['target']==geneA))]
								try:
